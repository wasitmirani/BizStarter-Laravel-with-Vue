...
import WarehouseAreaService from "@/Backend/Services/Warehouse/WarehouseAreaService";
...
const modalId = `warehouse-area-modal-${props.warehouseId}`;
...
const fetchData = async (page?: number, perPage?: number) => {
    ...
    try {
        const res = await WarehouseAreaService.index({
            warehouse_id: props.warehouseId,
            page: page ?? rows.value.current_page ?? 1,
            per_page: perPage ?? 15,
        });
        rows.value = res?.data?.result?.areas ?? rows.value;
    } catch (err: any) {
...
const onSubmit = async () => {
    ...
    try {
        ...
        if (isEditMode.value) {
           const res = await WarehouseAreaService.update(payload);
     
           toast.value?.showToast?.(res.status, 'Area Updated', res?.data);
        } else {
            const res = await WarehouseAreaService.store(payload);
            toast.value?.showToast(res.status, `Area created`, res.data?.message);
        }
...
const onAction = async ({ action, row }: any) => {
    ...
    if (action === "delete") {
        try {
            await WarehouseAreaService.delete(row.id);
            await fetchData();
        } catch (err: any) {
            toast.value?.showToast(err.status, `Error: ${err.status}`, err.response?.data?.message);
        }
    ...
    <div class="card">
        <div class="card-header flex items-center justify-between">
            ...
            <button
                ...
                @click="openCreate"
            >
                <i class="iconify tabler--plus"></i> Add Area
            </button>
        </div>
                ...
                <div class="border-default-300 pointer-events-auto flex flex-col rounded-md border card">
                    ...
                    <div class="overflow-y-auto card-body">
                        <form @submit.prevent="onSubmit" class="space-y-4">
                            <FormInput v-model="form.name" name="name" label="Name" placeholder="Area name" type="text" />
                            ...
                            <div class="border-default-300 flex items-center justify-end border-t pt-4">
                                ...
                                <button type="submit" class="btn bg-primary hover:bg-primary-hover m-1 rounded text-white" :disabled="isSaving">
                                    {{ isEditMode ? "Save changes" : "Save changes" }}
                                </button>
                            </div>
                        </form>
                    </div>
