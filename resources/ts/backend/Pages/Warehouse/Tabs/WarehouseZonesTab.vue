...
import WarehouseZoneService from "@/Backend/Services/Warehouse/WarehouseZoneService";
...
const modalId = `warehouse-zone-modal-${props.warehouseId}`;
...
const fetchData = async (page?: number, perPage?: number) => {
    ...
    try {
        const res = await WarehouseZoneService.index({
            warehouse_id: props.warehouseId,
            page: page ?? rows.value.current_page ?? 1,
            per_page: perPage ?? 15,
        });
        rows.value = res?.data?.result?.zones ?? rows.value;
    } catch (err: any) {
...
const onSubmit = async () => {
    ...
    try {
        ...
        if (isEditMode.value) await WarehouseZoneService.update(payload);
        else await WarehouseZoneService.store(payload);
...
const onAction = async ({ action, row }: any) => {
    ...
    if (action === "delete") {
        try {
            await WarehouseZoneService.delete(row.id);
            await fetchData();
        } catch (err: any) {
            toast.value?.showToast(err.status, `Error: ${err.status}`, err.response?.data?.message);
        }
...
<template>
    <div class="card">
        <div class="card-header flex items-center justify-between">
            <div class="font-semibold">Zones</div>
            <button
                type="button"
                ...
                @click="openCreate"
            >
                <i class="iconify tabler--plus"></i> Add Zone
            </button>
        </div>
                ...
                <div class="border-default-300 pointer-events-auto flex flex-col rounded-md border card">
                    ...
                    <div class="overflow-y-auto card-body">
                        <form @submit.prevent="onSubmit" class="space-y-4">
                            <FormInput v-model="form.name" name="name" label="Name" placeholder="Zone name" type="text" />
